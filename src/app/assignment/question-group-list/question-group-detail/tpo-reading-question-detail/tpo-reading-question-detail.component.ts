...
@Component({
	...
	styleUrls: ['./tpo-reading-question-detail.component.scss']
})
export class TpoReadingQuestionDetailComponent extends QuestionGroupDetailComponent
	...
	@ViewChild('passageContainer') passageContainer: any;
	...
	ngAfterViewChecked(): void {
		...
		if (this.passageContainer &&
			this.passageContainer.editorElem.children[0].children.length > 1) {
	...
	scroll(): void {
		...
		let paragraphNum = 0;
		let scrollTop = 0;
		for (let e of this.passageContainer.editorElem.children[0].children) {
			while (e.innerHTML.startsWith("♦")) {
		...
		for (let e of this.passageContainer.editorElem.children[0].children) {
			if (e.innerText.length > 1) {
				if (paragraphNum >= this.tpoReadingQuestion.paragraph) {
					...
					e.innerHTML = '<span class="star">♦</span>' +  e.innerHTML;
					break;
				}
		...
		if (this.tpoReadingQuestion.paragraph == 1) {
			this.passageContainer.editorElem.children[0].scrollTop = 0;
		} else {
			this.passageContainer.editorElem.children[0].scrollTop = scrollTop - 10;
		}
