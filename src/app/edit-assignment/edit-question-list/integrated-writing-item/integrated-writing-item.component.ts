		...
		this.dialog.open(EditIntegratedWritingQuestionDialogComponent, config).afterClosed()
			.pipe(
				...
				switchMap(formData =>{
					...
					return this.assignmentService.updateQuestionGroupContent(this.assignmentId, this.group.id, this.loadingContent);
				}),
				switchMap(data =>{
					return this.assignmentService.updateQuestion(this.loadingQuestion.id, this.loadingQuestion);
				})
			).subscribe(data=>{
